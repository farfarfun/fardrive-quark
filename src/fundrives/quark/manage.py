...
class QuarkPanManage:
    ...
    def request(self, uri, method='GET', params=None, headers=None, data=None, timeout=10, *args, **kwargs):
    ...
    def get_stoken(self, pwd_id: str) -> str:
        ...
        json_data = self.request('share/sharepage/token', 'post', data=data, )
    ...
    def get_detail(self, pwd_id: str, stoken: str, pdir_fid: str = '0', size=50,
                   sort='file_type:asc,updated_at:desc', ) -> Tuple[str, List[Dict[str, Union[int, str]]]]:
    ...
    def get_user_info(self) -> str:
        ...
        return requests.get('https://pan.quark.cn/account/info', params=params, headers=self.headers).json()
    ...
    def save_shared(self, share_url: str, folder_id: Union[str, None] = None, ) -> None:
        ...
        if data_list:
            ...
            logger.info(f'转存总数：{total_files_count}，文件数：{files_count}，文件夹数：{folders_count} | 支持嵌套')
            ...
            if not folder_id:
                logger.info('保存目录ID不合法，请重新获取，如果无法获取，请输入0作为文件夹ID')
                return

            if is_owner == 1:
                logger.info('网盘中已经存在该文件，无需再次转存')
                return
            task_id = self.get_share_save_task_id(pwd_id, stoken, fid_list, share_fid_token_list, to_pdir_fid=folder_id)
    ...
    def get_file_download_url(self, fid: str) -> None:
        ...
        if json_data['status'] != 200:
            logger.info(f"文件下载地址列表获取失败, {json_data['message']}", error_msg=True)
            return
        elif data_list:
            logger.info('文件下载地址列表获取成功')
        return data_list[0]['download_url']

    def get_share_save_task_id(self, pwd_id: str, stoken: str, first_ids: List[str], share_fid_tokens: List[str],
    ...
    def submit_task(self, task_id: str, retry: int = 50) -> Union[
        ...
        for i in range(retry):
            ...
            if json_data['message'] != 'ok':
                if json_data['code'] == 32003 and 'capacity limit' in json_data['message']:
                    logger.info("转存失败，网盘容量不足！请注意当前已成功保存的个数，避免重复保存", )
                elif json_data['code'] == 41013:
                    logger.info(f"网盘文件夹不存在，请重新运行按3切换保存目录后重试！", )
                else:
                    logger.info(f"错误信息：{json_data['message']}", )
                continue
            ...
            if json_data['data']['task_title'] == '分享-转存':
                ...
                logger.info(f"文件保存位置：{deep_get(json_data, 'data', 'save_as', 'to_pdir_name') or '根目录'} 文件夹")
            return json_data

    def get_share_task_id(self, fid: str, file_name: str, url_type: int = 1, expired_type: int = 2,
                          password: str = '') -> str:
        json_data = {
            "fid_list": [fid],
            "title": file_name,
    ...
    def submit_share(self, share_id: str) -> None:
        ...
        json_data = self.request('share/password', 'post', data=json_data, )
        share_url = json_data['data']['share_url']
        if 'passcode' in json_data['data']:
            share_url = share_url + f"?pwd={json_data['data']['passcode']}"
        return share_url

    def share(self, share_url: str, folder_id: Union[str, None] = None, url_type: int = 1, expired_type: int = 2,
              password: str = '') -> None:
        first_dir = ''
        second_dir = ''
        try:
            ...
            while True:
                json_data = self.get_file_list(pwd_id, page=first_page, size=50, fetch_total=True)
                for i1 in json_data['data']['list']:
                    ...
                    while True:
                        ...
                        json_data2 = self.get_file_list(i1['fid'], page=second_page, size=50, fetch_total=True, )
                        for i2 in json_data2['data']['list']:
                            ...
                            for i in range(3):
                                try:
                                    ...
                                    fid = i2['fid']
                                    task_id = self.get_share_task_id(fid, second_dir, url_type=url_type,
                                                                     expired_type=expired_type,
                                                                     password=password)
                                ...
                                if not share_success:
                                    ...
                                    logger.error(f'{error}.{first_dir}/{second_dir} 文件夹')
    ...
    def share_retry(self, retry_url: str, url_type: int = 1, expired_type: int = 2, password: str = ''):
    ...
    def search_file(self, file_name, page=1, size=50, sort='file_type:desc,updated_at:desc'):
    ...
    def get_file_list(self, pdir_fid='0', page=1, size=100, fetch_total=False, sort='file_type:asc,file_name:asc') -> \
            Dict[str, Any]:
        params = {
            'pdir_fid': pdir_fid,
            '_page': page,
